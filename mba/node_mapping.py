...
# Corrected cuneus/precuneus matching used by the validated pipeline.
_original_contains_any = contains_any
def corrected_contains(text, patterns):
    value = norm(text)
    for pattern in patterns:
        key = norm(pattern)
        if key == "cuneus":
            if "cuneus" in value and "precuneus" not in value:
                return True
        elif key in value:
            return True
    return False

def corrected_node_table(df, feature_cols, roi_set="disease_stage_cross_system"):
    global contains_any
    original = contains_any
    try:
        contains_any = corrected_contains
        X, names, table = build_node_table(df, feature_cols, roi_set)
    finally:
        contains_any = original
    if len(names) != len(set(names)):
        raise AssertionError("Duplicate anatomical node label")
    if "cuneus" in set(table.node):
        sources = table.set_index("node").loc["cuneus", "source_features"]
        if "precuneus" in str(sources).lower():
            raise AssertionError("cuneus/precuneus collision remains")
    return X, names, table

def overlap_audit(table):
    rows=[]
    for row in table.itertuples(index=False):
        for source in str(row.source_features).split(";"):
            rows.append({"node":row.node,"source_feature":source,
              "cuneus_precuneus_collision":bool(row.node=="cuneus" and "precuneus" in source.lower()),"status":"accepted"})
    return pd.DataFrame(rows)
